...
const PDFDisplay: React.FC<PDFDisplayProps> = React.memo(
    ({ file, pageNumber, zoomLevel, pageWidth, darkMode, onDocumentLoadSuccess, as }) => {
        ...
        if (error) {
            return (
                <div className="tw-flex tw-justify-center tw-items-center tw-min-h-[50vh]">
                    <div className="tw-text-center tw-text-red-500 tw-font-bold tw-text-xl">{error.message}</div>
                </div>
            );
        }
        ...
        return (
            ...
            >
                <div className={`tw-flex tw-justify-center tw-items-center ${darkMode ? 'dark-pdf' : ''}`}>
